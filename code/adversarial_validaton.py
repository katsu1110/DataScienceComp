...
class AdversarialValidation(object):
    ...
    def run(self):
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(self.train, self.train[target])):
            # train test split
            x_train, x_val = self.train[train_index, self.features], self.train[test_index, self.features]
            y_train, y_val = self.train[train_index, target], self.train[test_index, target]
        ...
        score = self.calc_metric(self.train[target], oof_pred)
        print('Our oof cv score is: ', score)
